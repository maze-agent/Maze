...
def _normalize_resources(resources: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    if normalized["gpu"] > 1:
        raise DagSpecError("resources.gpu must be at most 1")
    ...
    if target_node_id is not None and node_id is not None and str(target_node_id) != str(node_id):
        raise DagSpecError("resources.target_node_id and resources.node_id must match")
    target_node_id = target_node_id if target_node_id is not None else node_id
    if target_node_id is not None:
        target_node_id = str(target_node_id).strip()
        if not target_node_id:
            raise DagSpecError("resources.target_node_id must not be empty")
    ...
    if required_capability is not None:
        ...
        if not required_capability:
            raise DagSpecError("resources.required_capability must not be empty")
    ...
    if avoid_node_ids is not None:
        if not isinstance(avoid_node_ids, list):
            raise DagSpecError("resources.avoid_node_ids must be a list")
        normalized_avoid_node_ids = []
        for value in avoid_node_ids:
            node_id_value = str(value).strip()
            if not node_id_value:
                raise DagSpecError("resources.avoid_node_ids must not contain empty values")
        ...
        if target_node_id and target_node_id in normalized_avoid_node_ids:
            raise DagSpecError("resources.target_node_id cannot also be avoided")
