...
- **2026-06**: Maze added an external DAG submit protocol (`maze.workflow/v1`) so a decoupled visual platform can compile a canvas into one spec and run it through `POST /workflows/validate` and `POST /workflows/submit`, then observe execution via the unified run APIs.
- **2026-06**: Maze tasks support load-aware model degradation: a node can declare a primary plus a `fallback` implementation with lower resources, and the scheduler automatically switches to the backup when the primary cannot be placed within a pending timeout.
- **2026-06**: Maze added single-call LLM helpers for run analysis (`POST /runs/{run_id}/analyze`) and DAG design review (`POST /workflows/review`), each summarizing the run/DAG and asking an external OpenAI-compatible model for concrete suggestions.
- **2026-06**: Maze added DAG-level resume/rerun (`POST /runs/{run_id}/resume` re-runs only unfinished tasks, `POST /runs/{run_id}/rerun` re-runs from the persisted spec), per-task/agent performance aggregation (`GET /v1/agents/metrics`), structured failure localization with an error `stage` and `GET /runs/{run_id}/errors`, a namespaced context store (`/context/{namespace}/{key}`), and head/host system metrics (`GET /v1/system`).
