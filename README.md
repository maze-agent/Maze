...
**maze-sc** is the research artifact tree for **Maze**: a distributed, task-oriented framework for running LLM-heavy agent workloads at scale. The in-repo Python package is **`core`** (under `src/core/`); add the repository `src` directory to `PYTHONPATH` so `import core` resolves when running scripts by file path.
...
2. **HACS: heterogeneity-aware criticality scheduling**
  The default scheduler backend is **HACS** (`src/core/scheduler/hacs.py`), aligned with the paper’s value-density view: extract (\Omega, T_{\mathrm{pred}}, \Phi)style features per task, combine with **separate GPU / CPU / I/O dispatch paths** in `TaskScheduler` to reduce cross-resource head-of-line blocking, and use **online runtime prediction** (`exec_time_pred.py`, XGBoost-based) to inform ordering.
...
| Path                     | Role                                                                                                 |
| ------------------------ | ---------------------------------------------------------------------------------------------------- |
| `src/core/scheduler/` | Scheduler process (`scheduler.py`) and strategies (`hacs.py`, `fcfs.py`, `atlas.py`, `peft.py`).     |
| `src/core/resource/`  | Resource / worker side: `api_server.py`, `task_scheduler.py`, `standby_worker.py`, …                 |
| `src/core/workflows/` | Benchmark DAGs: **GAIA**, **τ-bench** (`tbench`), **OpenAGI** (`dag.json` + `task.py` per workflow). |
| `src/core/utils/`     | Loaders, execution backends, time prediction, helpers.                                               |
...
```bash
python src/core/resource/api_server.py \
  --redis_ip 127.0.0.1 --redis_port 6380 --flask_port 5000
```

`--proj_path` defaults to the **maze-sc root** inferred from `__file__` (directory that contains `src/core`). Override if your checkout layout is nonstandard.

### 3. Scheduler layer (HACS by default)

```bash
python src/core/scheduler/scheduler.py \
...
Shipped strategies in this tree are `**hacs**` (default), `**fcfs**`, `**atlas**`, and `**peft**`, each wired in `scheduler.py`. Adding a new strategy requires a corresponding `dag_manager_*` in `src/core/scheduler/` and a small change to the scheduler entrypoint.

### 4. Submit example DAGs

```bash
python src/core/agent/dispatch_task.py --master_addr 127.0.0.1:5001
```

The client posts to the scheduler Flask API (`/dag/`, `/status/`, `/get/`, `/release/`). See `src/core/agent/agent.py` for a thin Python wrapper.

## Other scheduling strategies

This tree ships `**hacs**` (default), `**fcfs**`, `**atlas**`, and `**peft**` under `src/core/scheduler/`. Pass `--strategy <name>` to `scheduler.py`; the entrypoint loads the matching `dag_manager_*` from that package.
