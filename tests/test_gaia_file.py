...
def test_artifact_snapshot_keeps_file_stable_across_attempts(monkeypatch, tmp_path):
    workspace = tmp_path / "workspace"
    ...
    worker_temp_root = tmp_path / "worker-artifact-temp"
    monkeypatch.setenv("MAZE_WORKER_ARTIFACT_TMP_DIR", str(worker_temp_root))
    observed_work_dirs = []
    ...
    assert all(worker_temp_root in work_dir.parents for work_dir in observed_work_dirs)
    assert all(not work_dir.exists() for work_dir in observed_work_dirs)
    assert list(worker_temp_root.iterdir()) == []
    assert [result["file_manifest"]["files"] for result in results] == [[], []]
