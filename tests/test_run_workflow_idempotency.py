...
def test_server_run_workflow_http_contract(monkeypatch):
    from maze.core import server

    payload = {
        "workflow_id": "template",
        "idempotency_key": "submission-api",
        "idempotency_fingerprint": FINGERPRINT,
    }
    ...
    async def reachable(_request, file_context):
        return file_context

    path = Path()
    monkeypatch.setattr(server, "mapath", path)
    monkeypatch.setattr(server, "_worker_reachable_file_context", reachable)

    response = asyncio.run(server.run_workflow(Request()))
    assert response == {
        "status": "success",
        "run_id": "run-api",
        "idempotency_key": "submission-api",
    ...
    assert path.calls[0][1]["idempotency_key"] == "submission-api"
    assert path.calls[0][1]["idempotency_fingerprint"] == FINGERPRINT

    payload.clear()
    payload["workflow_id"] = "template"
    assert asyncio.run(server.run_workflow(Request())) == {
        "status": "success",
        "run_id": "run-api",
    }

    def conflict(_workflow_id, **_kwargs):
        raise WorkflowIdempotencyConflictError(
            "submission-api",
            existing_run_id="run-api",
            existing_workflow_id="template",
            existing_fingerprint=FINGERPRINT,
        )

    path.run_workflow = conflict
    payload.update({
        "idempotency_key": "submission-api",
        "idempotency_fingerprint": FINGERPRINT,
    })
    ...
    assert error.value.detail["code"] == "workflow_idempotency_conflict"
    assert error.value.detail["existing_run_id"] == "run-api"

    def initialization_failure(_workflow_id, **_kwargs):
    ...
    path.run_workflow = initialization_failure
    with pytest.raises(HTTPException) as initialization_error:
        asyncio.run(server.run_workflow(Request()))
    assert initialization_error.value.status_code == 500
    assert initialization_error.value.detail == {
    ...
    def invalid_state(_workflow_id, **_kwargs):
        raise WorkflowIdempotencyStateError("stored claim is corrupt")

    path.run_workflow = invalid_state
    with pytest.raises(HTTPException) as state_error:
        asyncio.run(server.run_workflow(Request()))
    assert state_error.value.status_code == 500
    assert state_error.value.detail == {
        "code": "workflow_idempotency_state_invalid",
        "message": "stored claim is corrupt",
    }

    def missing_workflow(workflow_id, **_kwargs):
        raise WorkflowNotFoundError(workflow_id)

    path.run_workflow = missing_workflow
    payload["workflow_id"] = "missing-template"
    with pytest.raises(HTTPException) as not_found_error:
        asyncio.run(server.run_workflow(Request()))
    assert not_found_error.value.status_code == 404
    assert not_found_error.value.detail == {
        "code": "workflow_not_found",
        "message": "Workflow not found: missing-template",
        "workflow_id": "missing-template",
    }

    def invalid_submission(_workflow_id, **_kwargs):
        raise ValueError(
            "idempotency_key and idempotency_fingerprint must be provided together"
        )

    path.run_workflow = invalid_submission
    payload["workflow_id"] = "template"
    with pytest.raises(HTTPException) as bad_request_error:
        asyncio.run(server.run_workflow(Request()))
    assert bad_request_error.value.status_code == 400
    assert "must be provided together" in bad_request_error.value.detail

    payload.clear()
    with pytest.raises(HTTPException) as missing_id_error:
        asyncio.run(server.run_workflow(Request()))
    assert missing_id_error.value.status_code == 400
    assert missing_id_error.value.detail == "workflow_id is required"


@pytest.mark.parametrize("file_context", [[], "workspace", 1, True])
def test_server_rejects_non_object_file_context_with_400(
    monkeypatch,
    file_context,
):
    from maze.core import server
    ...
    class Path:
        def run_workflow(self, *_args, **_kwargs):
            raise AssertionError("Core must not receive malformed file_context")
    ...
    assert error.value.detail == "file_context must be an object"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"enabled": True},
        {
            "enabled": True,
            "workspace_dir": "/tmp/maze-invalid-file-context",
            "artifact_store": {"base_url": "not-an-absolute-url"},
        },
    ],
)
@pytest.mark.parametrize("with_idempotency", [False, True])
def test_server_rejects_nested_file_context_before_core_or_cluster_calls(
    monkeypatch,
    file_context,
    with_idempotency,
):
    ...
    class Path:
        ...
        def run_workflow(self, *args, **kwargs):
            ...
            raise AssertionError("Core must not claim a malformed submission")

        async def get_cluster_resources(self, *_args, **_kwargs):
            self.cluster_calls += 1
            raise AssertionError("Cluster lookup must follow submission validation")
