"""Ascend-Maze-native tau-bench retail cancellation/modify workflow."""
...
from workflows.tbench._common import (
    execute_retail_cancel_modify_operations,
    find_retail_user_for_modify,
    format_retail_cancel_modify_result,
    get_retail_cancel_modify_order_details_map,
    inference_features,
    load_retail_backend_data,
    metadata_dict,
    parse_cancel_modify_request,
    retail_cancel_modify_prompt,
)

SPEC = WorkflowSpec(
    name="maze-tbench-retail-cancel-modify",
    source="tbench",
    kind="retail_cancel_modify",
    nodes=nodes(
        (
            ("task0_init", "io", None),
            ...
            ("task3_execute_operations", "cpu", None),
    ...
    edges=edges(
        (
            ...
            ("task2a_find_user", "task3_execute_operations"),
            ("task2b_get_order_details", "task3_execute_operations"),
            ("task3_execute_operations", "task4_output_result"),
        )
    ),
)
...
def task0_init(
    ...
    prompt = retail_cancel_modify_prompt(question)
...
def task1_llm_process(
    ...
    operation_request = parse_cancel_modify_request(llm_output)
    features = {
...
def task2a_find_user(
    ...
    user_lookup = find_retail_user_for_modify(backend_data, operation_request)
    return {
        "dag_id": dag_id,
        "backend_data": backend_data,
        "operation_request": operation_request,
...
def task2b_get_order_details(
    ...
    order_details = get_retail_cancel_modify_order_details_map(
        backend_data,
        operation_request,
    )
    return {
...
def task3_execute_operations(
    ...
    if not isinstance(details_map, dict):
        details_map = {}
    final_result = execute_retail_cancel_modify_operations(
        backend_data,
        operation_request,
        user_lookup,
        details_map,
...
def task4_output_result(
    ...
    final_output = format_retail_cancel_modify_result(final_result)
    return {
        "dag_id": dag_id,
        "status": final_result.get("status", "error"),
        "result": final_output,
...
def build() -> Workflow:
    ...
    user = workflow.add_task(
        ...
        inputs={
            ...
            "operation_request": extracted.outputs["operation_request"],
        },
    )
    ...
    executed = workflow.add_task(
        task3_execute_operations,
        task_name="task3_execute_operations",
        inputs={
            "dag_id": user.outputs["dag_id"],
            "backend_data": user.outputs["backend_data"],
            "operation_request": user.outputs["operation_request"],
