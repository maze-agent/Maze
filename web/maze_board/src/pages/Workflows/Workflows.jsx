...
const Workflows = ({ workflows, onWorkflowClick, onServiceAction }) => {
  ...
  const handleServiceAction = (e, workflowId, action) => {
    e.stopPropagation(); // 防止触发 view details
    if (onServiceAction) {
      onServiceAction(workflowId, action);
    }
  };
    ...
    return (
      <div className="flex gap-2">
        {workflow.service_status === 'running' && (
          <button
            onClick={(e) => handleServiceAction(e, workflow.workflow_id, 'pause')}
        ...
        {workflow.service_status === 'paused' && (
          <>
            <button
              onClick={(e) => handleServiceAction(e, workflow.workflow_id, 'resume')}
              className={`${baseButtonClass} bg-green-600 hover:bg-green-700`}
            >
              Resume
            </button>
            <button
              onClick={(e) => handleServiceAction(e, workflow.workflow_id, 'stop')}
        ...
        {workflow.service_status === 'stopped' && (
          <button
            onClick={(e) => handleServiceAction(e, workflow.workflow_id, 'start')}
            className={`${baseButtonClass} bg-green-600 hover:bg-green-700`}
          >
            Start
          </button>
