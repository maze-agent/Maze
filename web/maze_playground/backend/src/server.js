...
function compactRunDiagnosticText(value, maxLength = 420) {
  let text = '';
  if (typeof value === 'string') {
    text = value;
...
function applyStaticRunEvent(snapshot, event) {
  ...
  if (event.type === 'workflow_started') {
    ...
    snapshot.finished_time = snapshot.finished_time || eventTime;
    snapshot.error = compactRunDiagnosticText(data.error || 'Workflow failed', 2000);
    ...
    node.status = 'failed';
    node.finished_time = eventTime;
    node.error = compactRunDiagnosticText(data.error || data.result || 'Task failed', 2000);
    node.maze_task_id = data.maze_task_id || data.task_id || node.maze_task_id;
    snapshot.status = 'failed';
    snapshot.finished_time = snapshot.finished_time || eventTime;
    ...
    (async () => {
      try {
        ...
        if (!result.success) {
          ...
          workflow.error = compactRunDiagnosticText(result.error || 'Workflow failed', 2000);
          workflows.set(id, workflow);
        ...
        if (latestRunAfterPython?.status === 'failed') {
          ...
          workflow.status = 'failed';
          workflow.error = compactRunDiagnosticText(latestRunAfterPython.error || 'Workflow task failed', 2000);
