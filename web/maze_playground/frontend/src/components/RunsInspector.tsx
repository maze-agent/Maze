...
function issueGuidance(issue: any) {
  ...
  let stage = 'runtime';
  let suggestion = 'Open the failed task or event, inspect the raw error, then retry after fixing the input or environment.';

  if (lower.includes('api key') || lower.includes('unauthorized') || lower.includes('401')) {
    stage = 'llm';
    suggestion = 'Check Advanced Setting -> LLM API key/base URL/model, then rerun the workflow.';
    ...
    stage = 'execution';
    suggestion = 'Increase timeout or split the work into a smaller task before rerunning.';
    ...
    stage = 'input';
    suggestion = 'Inspect the raw payload and task parser. Rerun after making the input schema more explicit.';
  }
      ...
      <div>
        ...
        >
          ...
          <div style={{ border: '1px solid #f0f0f0', borderRadius: 6, padding: 10, background: '#fff' }}>
            ...
            <Space size={[4, 4]} wrap style={{ display: 'flex', marginTop: 8 }}>
              <Tag color={events.length > 0 ? 'blue' : 'default'}>{events.length} event(s)</Tag>
              <Tag color={nodes.length > 0 ? 'geekblue' : 'default'}>{nodes.length} task node(s)</Tag>
            </Space>
          </div>
