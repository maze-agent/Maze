...
class Task:
    ...
    def status(self):
        ...
        response = requests.post(f"http://{_svc.task_center_addr}/status/", json=payload)
        res = response.json()
        if res["status"] == "error":
            print(res["msg"])
        else:
    ...
    def get(self, func_name):
        ...
        response = requests.post(f"http://{_svc.task_center_addr}/get/", json=payload)
        res = response.json()
        if res["status"] == "error":
            print(res["msg"])
        else:
    ...
    def release(self):
        ...
        response = requests.post(f"http://{_svc.task_center_addr}/release/", json=payload)
        res = response.json()
        if res["status"] == "error":
            print(res["msg"])
        else:
...
class Core_:
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    cls._instance = super(Core_, cls).__new__(cls)
...
def draw(folder: str):
    ...
    pos = {
        node: (i, -j)
        for j, layer in enumerate(nx.topological_generations(G))
...
def submit(folder: str):
    ...
    response = requests.post(f"http://{_svc.task_center_addr}/dag/", json=payload)
    res = response.json()
