...
def decode_audio_to_array(audio_bytes: bytes, target_sr: int = 16000):
    ...
    Numpy
    """
    print("currentlyusepydub...")
    audio_segment = AudioSegment.from_file(BytesIO(audio_bytes))
    
    wav_buffer = BytesIO()
    audio_segment.export(wav_buffer, format="wav")
    wav_buffer.seek(0)
    print("WAVcurrentlyuselibrosa...")
...
def task1_speech_process(context):
    ...
    try:
        ...
        # --- 2.  ---
        time_record["get_size_bytes"] = len(cloudpickle.dumps(supplementary_files))
        ...
        print(f"audio feature extraction done: {audio_features}")

        # --- 3.  put  ---
        time_record["put_size_bytes"] += len(cloudpickle.dumps(audio_array))
        context.put.remote("file_content", audio_array)
        
        time_record["put_size_bytes"] += len(cloudpickle.dumps(audio_features))
...
def task2_speech_process(context):
    ...
    try:
        ...
        time_record["get_size_bytes"] = sum(
            ...
            [dag_id, question, content, model_folder, audio_features]
        )
        ...
        print("🔊 Whisper ", processed_content)
        ...
        print(f"task2_speech_recognition error: {str(e)}")
        import traceback
        print(f"task2_speech_recognition : {traceback.format_exc()}")
        raise e
...
def task3_llm_process_qwen(context):
    ...
    try:
        ...
        if use_online_model:
            ...
            vllm_base = ray.get(context.get.remote("task3_llm_process_qwen_request_api_url"))
        route_meta = (
        ...
        print(f"task3_llm_process_qwen error: {str(e)}")
        raise e
...
def task4_llm_process_deepseek(context):
    ...
    try:
        ...
        if use_online_model:
            ...
            vllm_base = ray.get(context.get.remote(f"task4_llm_process_deepseek_request_api_url"))
        route_meta = (
            hosted_route.redacted_log_hint() if hosted_route else None,
            RemoteLlmRoute.for_vllm_worker_base(vllm_base).redacted_log_hint() if vllm_base else None,
        )
        ...
        print(f"task4_llm_process_deepseek error: {str(e)}")
        raise e
...
def task5_llm_fuse_answer(context):
    ...
    try:
        ...
        if use_online_model:
            ...
            vllm_base = ray.get(context.get.remote(f"task5_llm_fuse_answer_request_api_url"))
        route_meta = (
            hosted_route.redacted_log_hint() if hosted_route else None,
            RemoteLlmRoute.for_vllm_worker_base(vllm_base).redacted_log_hint() if vllm_base else None,
        )
        ...
        # ---  ---
        computation_start_time = time.time()
        print("✅ Task 5 (Fusion): Starting answer synthesis...")
